...
#define SFT_SDA_PIN					11		/* PIN14_PD7_D11 */
#define SFT_SCL_PIN					10		/* PIN13_PD6_D10 */
...
void check_sensor(RTCDRV_TimerID_t id, void *user)
{
	...
	if (sample_count % sample_period == 0) {
		...
		ret = push_point(&g_cbuf, pcf8563_now(), (cur_pres * 100), fetch_mcu_temp());
		#else
		ret = push_point(&g_cbuf, seconds(), (cur_pres * 100), fetch_mcu_temp());
		#endif
...
#ifdef ENABLE_RTCx
	push_point(&g_cbuf, pcf8563_now(), (cur_pres * 100), fetch_mcu_temp());
#else
	push_point(&g_cbuf, seconds(), (cur_pres * 100), fetch_mcu_temp());
#endif
}
			...
			while (get_1st_point(&g_cbuf, &d) == 0 && cnt_fail < 15) {
				...
				if (tx_cause == 0 || gmtime(&(d.ts))->tm_hour == 12) {
					...
					sprintf(modem_said, MQTT_INIT_MSG,
							...
							devid,
							decode_vbat(vbat),
							decode_sensor_data(d.data / 100.0),
							d.iT,
							tx_cause,
							iccid
					);
