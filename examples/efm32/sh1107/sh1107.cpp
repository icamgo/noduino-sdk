...
void sh1107::setContrastLevel(unsigned char ContrastLevel) {
    sendCommand(sh1107_Set_ContrastLevel_Cmd);
...
void sh1107::putChar(unsigned char C) {
...
void sh1107::putString(const char* String) {
    unsigned char i = 0;
    while (String[i]) {
        putChar(String[i]);
        i++;
...
unsigned char sh1107::putNumber(long long_num) {
...
void sh1107::drawBitmap(const unsigned char* bitmaparray, int bytes) {
    if (Drive_IC == SSD1327) {
...
void sh1107::setHorizontalScrollProperties(bool direction, unsigned char startRow, unsigned char endRow,
...
void sh1107::activateScroll() {
    sendCommand(sh1107_Activate_Scroll_Cmd);
}

void sh1107::deactivateScroll() {
    sendCommand(sh1107_Dectivate_Scroll_Cmd);
}

void sh1107::setNormalDisplay() {
    if (Drive_IC == SSD1327) {
        sendCommand(sh1107_Normal_Display_Cmd);
    } else if (Drive_IC == SH1107G) {
        sendCommand(sh1107_Normal_Display_Cmd_SH1107G);
    }
}

void sh1107::setInverseDisplay() {
    sendCommand(sh1107_Inverse_Display_Cmd);
}

sh1107 sh1107_oled;  // Preinstantiate Objects
